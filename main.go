...
func main() {
	...
	opts.Run(func(gen *protogen.Plugin) error {
		...
		for _, f := range gen.Files {
			...
			if err := generateCLIFile(gen, f, config); err != nil {
				return err
			}
...
func generateCLIFile(plugin *protogen.Plugin, file *protogen.File, config *model.Config) error {
	// Collect services that opted in to CLI generation. If none, skip the
	// file entirely so packages without generate_cli=true don't get an
	// empty _tmprl_cli.pb.go sitting next to the main generated file.
	var cliServices []*protogen.Service
	for _, s := range file.Services {
		so, ok := proto.GetExtension(s.Desc.Options(), temporalv1.E_Service).(*temporalv1.ServiceOptions)
		if !ok || so == nil {
			continue
		}
		if so.GetGenerateCli() {
			cliServices = append(cliServices, s)
		}
	}
	...
	filename := file.GeneratedFilenamePrefix + "_tmprl_cli.pb.go"
	...
	for _, s := range cliServices {
		...
		if err != nil {
			return fmt.Errorf("failed to create service model for CLI: %w", err)
		}
		...
		output, err := r.RenderCLI(service)
		if err != nil {
			return fmt.Errorf("failed to render CLI for service %s: %w", service.GoName, err)
		}
	...
	if err != nil {
		return fmt.Errorf("failed to format CLI code: %w", err)
	}
