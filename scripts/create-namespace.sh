#!/bin/sh
# Creates the default namespace once the server is healthy. Taken from
# temporalio/samples-server compose/scripts, with retention support added
# (the old auto-setup image handled this via DEFAULT_NAMESPACE_RETENTION).
# TLS towards the frontend is picked up from the TEMPORAL_TLS_* env vars.
set -eu
...
while :; do
  ...
  if temporal operator namespace create -n "$NAMESPACE" --retention "$RETENTION" --address "$TEMPORAL_ADDRESS" >/dev/null 2>&1; then
